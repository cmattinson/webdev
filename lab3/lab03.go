...
//
// Lab 03: PostgreSQL, goose, and jmoiron/sql
...
func listCourses(db *sqlx.DB) error {
	rows, err := db.Query("SELECT course_id, course_number FROM course")

	for rows.Next() {
		var course_id string
		var course_number int

		err = rows.Scan(&course_id, &course_number)
	}
...
func connectToDB() (*sqlx.DB, error) {
	...
	db, err := sqlx.Connect("postgres", "user=postgres dbname=lab03 sslmode=disable")
